...
def classificationDataLoader(fileName, labelCol=-1, dtype='int32', delimiter=',', sparsify=False):
    '''
        ...
        :param
            ...
            dtype:      np date type, default 'int32'
            delimiter:  feature seperator each sample, default ','
            sparsify:   whether to return sparsify X
        :returns
            X:  np feature matrix   (n_samples, n_features)
            Y:  np label matrix     (n_samples, 1)
    '''
    data = np.loadtxt(fileName, dtype=dtype, delimiter=delimiter)
...
# X, Y = classificationDataLoader('dataset/test.data', sparsify=True)

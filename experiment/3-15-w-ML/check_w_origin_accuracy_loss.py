...
class Logistic_Regression(object):
    ...
    def fit(self, xTrain, yTrain, w, verbos=False):
        ...
        self.w = np.zeros((self.featureNum+1, 1))#np.zeros(shape=(self.featureNum+1, 1), dtype='float32')
        self.w[:-1,0] = np.copy(w).reshape(self.featureNum)# random.normal(0, 0.01, size=(self.featureNum+1, 1))#np.zeros(shape=(self.featureNum+1, 1), dtype='float32')
            ...
            while True:
                ...
                if iter % 100 == 0:
                    ...
                    if verbos:
                        ...
                        print "w norm %10.6f\t|\tdelta_w norm %10.6f\t|\tw_loss %10.10f"%(np.linalg.norm(self.w, ord=2), np.linalg.norm(self.w_lr(epoch_num) * delta_w, ord=2), self.w_loss())
...
# w loss
def w_loss(w, reg_lambda, pi, gm_num):
    responsibility = gaussian.pdf(w, loc=np.zeros(shape=(1, gm_num)), scale=1/np.sqrt(reg_lambda))*pi
    responsibility_w = np.sum(responsibility, axis=1)
    log_responsibility_w = -np.log(responsibility_w)
    return np.sum(log_responsibility_w)
...
if __name__ == '__main__':
    ...
    # w_norm
    # origin accuracy
    simulator = pickle.load(open('simulator.pkl', 'r'))
    lg = np.dot(simulator.x, simulator.w)
    print "origin optimal accuracy: ", np.sum((lg >= 0) != simulator.label) / float(simulator.sample_num)

    print "train accuracy: ", accuracy(np.matmul(xTrain, w)>0.0, yTrain)

    reg_lambda = np.array([200, 200, 10, 1.25]).reshape((1,-1))
    pi = np.array([0.70, 0.05, 0.2, 0.05]).reshape((1,-1))
    gm_num = 4
    print "w loss: ", w_loss(w, reg_lambda, pi, gm_num)

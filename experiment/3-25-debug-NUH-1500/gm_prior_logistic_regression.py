...
if __name__ == '__main__':
    ...
    for i, (train_index, test_index) in enumerate(StratifiedKFold(y.reshape(y.shape[0]), n_folds=n_folds)):
        if i > 0:
            break
        print "train_index shape, train_index: ", train_index.shape, train_index
        print "test_index shape, test_index: ", test_index.shape, test_index
        ...
        pi, reg_lambda,  eps, batch_size \
            = [1.0/gm_num for _ in range(gm_num)], [np.power(0.5, (_+1)) for _ in  range(gm_num)], 1e-10, args.batchsize
...
# command python gm_prior_logistic_regression.py -datapath LACE-CNN-1500-lastcase.pkl -onehot 1 -sparsify 0 -batchsize 30 -wlr 2 -pirlr 5 -lambdaslr 3 -maxiter 700 -gmnum 3 -a 1 -b 10 -alpha 1 -gmoptmethod 1 | tee -a 3-25-debug-NUH-1500

...
class Simulator():
    ...
    def wGenerator(self):
        ...
        self.w_origin = np.random.choice(self.gm_num, size=(self.dimension), p=self.pi)
...
if __name__ == '__main__':
    dimension, sample_num = 1000, 10000
    ...
    if gm_num == 2:
        for i in range((len(pi_variance_array_2) + 1)/2):
            pi, variance, covariance = pi_variance_array_2[2 * i], pi_variance_array_2[2 * i + 1], np.identity(dimension)#generateCov(n=dimension, a=1, showCov=False)
            if sum(pi) != 1:
                continue
            print "simulation pi, variance, sum(pi): ", pi, variance, sum(pi)
            ...
            with open('simulator2-' + str(i) + '.pkl', 'w') as saveFile:
                pickle.dump(simulator, saveFile)

    if gm_num == 4:
        for i in range((len(pi_variance_array_4) + 1)/2):
            pi, variance, covariance = pi_variance_array_4[2 * i], pi_variance_array_4[2 * i + 1], np.identity(dimension)#generateCov(n=dimension, a=1, showCov=False)
            if sum(pi) != 1:
                continue
            print "simulation pi, variance, sum(pi): ", pi, variance, sum(pi)
            simulator = Simulator(gm_num, dimension, sample_num, pi, variance, covariance)
            ...
            simulator.labelGenerator(noiseVar=1.2)
            # save the generated simulator
            with open('simulator4-' + str(i) + '.pkl', 'w') as saveFile:
                pickle.dump(simulator, saveFile)

    if gm_num == 8:
        for i in range((len(pi_variance_array_8) + 1)/2):
            pi, variance, covariance = pi_variance_array_8[2 * i], pi_variance_array_8[2 * i + 1], np.identity(dimension)#generateCov(n=dimension, a=1, showCov=False)
            if sum(pi) != 1:
                continue
            print "simulation pi, variance, sum(pi): ", pi, variance, sum(pi)
            simulator = Simulator(gm_num, dimension, sample_num, pi, variance, covariance)
            ...
            simulator.labelGenerator(noiseVar=1.2)
            # save the generated simulator
            with open('simulator8-' + str(i) + '.pkl', 'w') as saveFile:
                pickle.dump(simulator, saveFile)

...
'''
'''
for gm_num = 4 or 8, split according to pi_variance_array
'''
import sys
...
if __name__ == '__main__':
    ...
    for i in range(3,6):
        ...
        reg_lambda, learning_rate, max_iter, eps, batch_size = np.linspace(reg_lambda_min, reg_lambda_max, num=8), 0.0001, 1000000, 1e-10, 500
        ...
        for reg in reg_lambda:
            ...
            print "load data path: ", ('simulator' + str(gm_num) + '-' + str(i) + '.pkl')
            print LG
            np.savetxt('weight-out/'+sys.argv[0][:-3]+'_L2_w_' + str(gm_num) + '_' + str(i) + '_' + str(reg) + '.out', LG.w, delimiter=',')
    #
    # fig, ax = plt.subplots()

...
def branin(reg_lambda_value, l1_ratio_value, datapath, onehot, sparsify, batchsize, wlr, maxiter):
    print ('brain.py brain() reg_lambda_value: ', reg_lambda_value)
    print ('brain.py brain() l1_ratio_value: ', l1_ratio_value)
        ...
        for l1_ratio in l1_ratio_array:
            for reg in reg_lambda:
                ...
                if not np.isnan(np.linalg.norm(LG.w)):
                    ...
                    accuracy_df.loc[i, (str(l1_ratio) + ',' + str(reg))] = LG.accuracy(LG.predict(xTest, (sparsify==1)), yTest)
                    auc_df.loc[i, (str(l1_ratio) + ',' + str(reg))] = LG.auroc(LG.predict_proba(xTest, (sparsify==1)), yTest)
                    loss_df.loc[i, (str(l1_ratio) + ',' + str(reg))] = (LG.loss(xTest, yTest, (sparsify==1)) * xTest.shape[0])
                print LG
...
def main(job_id, params):
    ...
    datapath = "/home/singa/zhaojing/smooth-regularization-dbpcm-bak-logistic/spearmint/Spearmint/examples/data/breast-cancer-wisconsin-normed.pkl"
    onehot = 0
    sparsify = 0
    batchsize = 30
    wlr = 4

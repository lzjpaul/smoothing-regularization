...
def branin(reg_lambda_value, reg_mu_value, datapath, onehot, sparsify, batchsize, wlr, maxiter):
    ...
    print ('brain.py brain() reg_mu_value: ', reg_mu_value)
        ...
        for mu_val in reg_mu:
            for lambda_val in reg_lambda:
                ...
                if not np.isnan(np.linalg.norm(LG.w)):
                    ...
                    accuracy_df.loc[i, (str(mu_val) + ',' + str(lambda_val))] = LG.accuracy(LG.predict(xTest, (sparsify==1)), yTest)
                    auc_df.loc[i, (str(mu_val) + ',' + str(lambda_val))] = LG.auroc(LG.predict_proba(xTest, (sparsify==1)), yTest)
                    loss_df.loc[i, (str(mu_val) + ',' + str(lambda_val))] = (LG.loss(xTest, yTest, (sparsify==1)) * xTest.shape[0])
                print LG

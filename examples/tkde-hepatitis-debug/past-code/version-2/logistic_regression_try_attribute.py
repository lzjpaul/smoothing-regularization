...
def main():
    ...
    x, y = loadData('../data/hepatitis_normed.pkl', onehot=(0==1), sparsify=(0==1))
    ...
    for i, (train_index, test_index) in enumerate(StratifiedKFold(y.reshape(y.shape[0]), n_folds=n_folds)):
        ...
        for reg in reg_lambda:
            ...
            learning_rate, max_iter = math.pow(10, (-1 * 4)), 1200
            eps, batch_size = 1e-10, 30
            print "\nreg_lambda: %f" % (reg)
            LG = Logistic_Regression(reg, learning_rate, max_iter, eps, batch_size)
            LG.fit(xTrain, yTrain, xTest, yTest, (0==1), gm_opt_method=-1, verbos=True)
            if not np.isnan(np.linalg.norm(LG.w)):
                print "\n\nfinal accuracy: %.6f\t|\tfinal auc: %6f\t|\ttest loss: %6f" % (LG.accuracy(LG.predict(xTest, (0==1)), yTest), \
                                                               LG.auroc(LG.predict_proba(xTest, (0==1)), yTest), LG.loss(xTest, yTest, (0==1)))
                accuracy_df.loc[i, (str(reg))] = LG.accuracy(LG.predict(xTest, (0==1)), yTest)
                auc_df.loc[i, (str(reg))] = LG.auroc(LG.predict_proba(xTest, (0==1)), yTest)
                loss_df.loc[i, (str(reg))] = (LG.loss(xTest, yTest, (0==1)) * xTest.shape[0])
            print LG

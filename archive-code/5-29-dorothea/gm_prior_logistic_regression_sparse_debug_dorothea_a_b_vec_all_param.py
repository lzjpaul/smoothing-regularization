...
class GM_Logistic_Regression(Logistic_Regression):
    def __init__(self, hyperpara, gmm_update_frequency, gm_num, pi, reg_lambda, learning_rate=0.1, pi_r_learning_rate=0.1, reg_lambda_s_learning_rate=0.1, max_iter=1000, eps=1e-4, batch_size=-1, validation_perc=0.0):
        ...
        self.a = np.asarray([1.001, self.a])
        self.b = np.asarray([self.b * 5000., self.b])
        self.alpha = np.array([self.alpha, 100000**(0.1)])
    ...
    def update_GM_Prior_EM(self, epoch_num, iter_num):
        ...
        if iter_num % 100 == 0 or iter_num < 100:
            ...
            print "self.a, self.b, self.alpha: ", self.a, self.b, self.alpha
            ...
            print "division: ", np.sum(self.responsibility * np.square(self.w[:-1]), axis=0) / np.sum(self.responsibility, axis=0)
        # update reg_lambda
        # self.pi = (np.sum(self.responsibility, axis=0) + self.alpha - 1) / (self.featureNum + self.gm_num * (self.alpha - 1))
        self.pi = (np.sum(self.responsibility, axis=0) + self.alpha - 1) / (self.featureNum + np.sum(self.alpha - 1))
        print "(np.sum(self.responsibility, axis=0) + self.alpha - 1): ", (np.sum(self.responsibility, axis=0) + self.alpha - 1)
        print "(self.featureNum + np.sum(self.alpha - 1)): ", (self.featureNum + np.sum(self.alpha - 1))
    ...
    # model parameter
    def __str__(self):
        return 'model config {\thyper: [a-%s,b-%s,alpha-%s] reg: %s, pi: %s, lr: %.6f, pi_r_lr: %.6f, reg_lambda_s_lr: %.6f, batch_size: %5d\t}' \
...
if __name__ == '__main__':
    ...
    for i, (train_index, test_index) in enumerate(StratifiedKFold(y.reshape(y.shape[0]), n_folds=n_folds)):
        ...
        b, alpha = [(fea_num  * 1e-5)], \
                   [3300000, 3500000, 3700000]
        for alpha_val in alpha:
            for b_val in b:
                # a = [(1. + b_val * 1e-1), (1. + b_val * 1e-2), (1. + b_val * 1e-3)]
                a = [(b_val * 1e-3 * 28000000. + 1.), (b_val * 1e-3 * 30000000. + 1.), (b_val * 1e-3 * 32000000. + 1.)]
                for a_val in a:
                    start = time.time()

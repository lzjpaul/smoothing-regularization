...
class Gaussian_Mixture_Regularization(BaseEstimator, LogisticLinearClassifierMixin):
    ...
    def fit(self, X_train, y_train, X_test, y_test, batchgibbs):
        ...
        self.n_iter_, self.w_, self.best_accuracy_, self.best_accuracy_step_ = optimizator_gd.gaussian_mixture_optimizator_avg(X_train, y_train, X_test, y_test, self.C,
                                                     (y_train.shape[0] * 10 / self.batch_size), self.eps, self.alpha, self.n_gaussian, self.theta_alpha, self.a, self.b, self.decay, self.batch_size, 'gaussianmixture', batchgibbs)

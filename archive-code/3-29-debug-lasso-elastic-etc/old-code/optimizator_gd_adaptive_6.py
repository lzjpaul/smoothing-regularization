# optimizator-6.py: adaptive tune one gaussian
# (0) overall logic
...
def gaussian_mixture_optimizator_avg(X_train, y_train, X_test, y_test, C, max_iter, eps, alpha, n_gaussian, theta_alpha, a, b, decay, batch_size, clf_name, batchgibbs, initial_L2_lambd, initial_L2_alpha, initial_L2_step):
    ...
    while True:
        ...
        if k == 60:
            file = open('step60_weight.csv', "w")
            np.savetxt(file, w.toarray(), "%5f", ",")
            file.close()
            file = open('step60_weight_update.csv', "w")
            np.savetxt(file, w_update.toarray(), "%5f", ",")
            file.close()

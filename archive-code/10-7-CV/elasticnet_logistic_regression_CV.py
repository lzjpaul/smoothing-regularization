...
if __name__ == '__main__':
    ...
    x_all, y_all = loadData(args.datapath, onehot=(args.onehot==1), sparsify=(args.sparsify==1))
    print "loadData x shape: ", x_all.shape
    print "y_all shape: ", y_all.shape
    outer_n_folds = 5
    for j, (train_outer_index, test_outer_index) in enumerate(StratifiedKFold(y_all.reshape(y_all.shape[0]), n_folds=outer_n_folds)):
        print "outer loop index: ", j
        x, y = x_all[train_outer_index], y_all[train_outer_index]
        print "outer loop x train shape: ", x.shape
        print "outer loop y train shape: ", y.shape
        n_folds = 3
            ...
            for l1_ratio in l1_ratio_array:
                for reg in reg_lambda:
                    ...
                    if not np.isnan(np.linalg.norm(LG.w)):
                        ...
                        accuracy_df.loc[i, (str(l1_ratio) + ',' + str(reg))] = LG.accuracy(LG.predict(xTest, (args.sparsify==1)), yTest)
                        auc_df.loc[i, (str(l1_ratio) + ',' + str(reg))] = LG.auroc(LG.predict_proba(xTest, (args.sparsify==1)), yTest)
                        loss_df.loc[i, (str(l1_ratio) + ',' + str(reg))] = (LG.loss(xTest, yTest, (args.sparsify==1)) * xTest.shape[0])
                    print LG
        ...
        print "check accuracy max mean index: ", accuracy_df.mean().idxmax()

        ...
        for alpha_val in alpha:
            for a_val in a:
                for b_val in b:
                    ...
                    LG.fit(x[train_index], y[train_index], (args.sparsify==1), gm_opt_method=gm_opt_method, verbos=True)
                    print "\n\nfinal accuracy: %.6f\t|\tfinal auc: %6f" % (LG.accuracy(LG.predict(x[test_index], (args.sparsify==1)), y[test_index]), \
                                                                           LG.auroc(LG.predict_proba(x[test_index], (args.sparsify==1)), y[test_index]))
                    print LG

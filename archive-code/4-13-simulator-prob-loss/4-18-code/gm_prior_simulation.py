...
class Simulator():
    ...
    def wGenerator(self):
        ...
        self.w_origin = np.random.choice(3, size=(self.dimension), p=self.pi)
    ...
    def labelGenerator(self, noiseVar=0.1):
        ...
        with open('generated_y_vals_complete.dta', 'w') as saveFile:
...
if __name__ == '__main__':
    gm_num, dimension, sample_num = 3, 1000, 50000
    pi, variance, covariance = np.array([0.75, 0.2, 0.05]), np.array([0.005, 0.1, 0.8]), np.identity(dimension)#generateCov(n=dimension, a=1, showCov=False)
